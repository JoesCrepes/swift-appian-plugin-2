...
public class SwiftMtConverterFunctions {
   ...
   * @param useStructuredFormat
   *          Optional. If true (default), uses structured JSON
  ...
  public String swiftmttojson(
    ...
    @Parameter(required = false) Boolean useStructuredFormat) throws AppianException {
    try {
   ...
   *          The raw SWIFT MT message string
   * @param messageType
   *          The expected MT message type (e.g., "103", "MT103", "202")
   * @return JSON string representation of the message
   * @throws AppianException
  ...
  public String swiftmttojsonbytype(
    @Parameter String mtMessage,
    @Parameter String messageType) throws AppianException {
    try {
      ...
      throw new AppianException(
        "Unexpected error converting SWIFT MT" + messageType + " message to JSON: " + e.getMessage(),
        e);
    }
  }
}
